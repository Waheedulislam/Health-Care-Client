...
export default function MultipleSelectFiledChip({ schedule, setSelectedScheduleIds, selectedScheduleIds }: any) {
                        ...
                        return (
                            <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
                                {selected.map((value: any) => {
                                    const selectedSchedule = schedule.find((schedule: any) => schedule.id === value);
                                    if (!selectedSchedule) return null;

                                    const formattedTimeSlot = `${getTimeIn12HourFormat(selectedSchedule.startDate)}
                                    -
